...
class EmbededHybridNetv2(nn.Module):

    def __init__(self, num_user: int, genres, titles_and_plots, directors_and_casts):
        super(EmbededHybridNetv2, self).__init__()
        # Item features
        self.item_weighted_genres = FeatureGenerator(genres)
        self.item_titles_plots = FeatureGenerator(titles_and_plots)
        self.item_directors_casts = FeatureGenerator(directors_and_casts)
        ...
        self.dense_titles_plots = nn.Linear(self.item_titles_plots.num_feature * 2, 20)
        self.dense_directors_casts = nn.Linear(
            self.item_directors_casts.num_feature * 2, 10
        )
        # Dense
        self.out = nn.Linear(32, 1, dtype=torch.float32)
        # Relu
        self.relu = nn.ReLU()
    ...
    def __call__(self, user, item):
        ...
        directors_casts = self.dense_directors_casts(directors_casts)
        # Final denses
        x = torch.cat([weighted_genres, titles_plots, directors_casts], dim=1)
        x = self.relu(x)
        return self.out(x)
